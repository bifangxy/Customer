...
public class PickerScrollView extends View {
    ...
    private Pickers select_pickers;

    private float showCount = 4.0f;
    ...
    private void performSelect() {
        select_pickers = mDataList.get(mCurrentSelected);
        if (mSelectListener != null)
            mSelectListener.onSelect(mDataList.get(mCurrentSelected));
    }

    public Pickers getSelectData() {

        return mDataList.get(mCurrentSelected);
    }

    public void setShowCount(float count) {
        showCount = count;
    }

    public void setPainColor(int mColorText) {
    ...
    public void setSelectedContent(String Content) {
        for (int i = 0; i < mDataList.size(); i++)
            if (mDataList.get(i).getShowConetnt().equals(Content)) {
                setSelected(i);
                break;
            }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 按照View的高度计算字体大小
        mMaxTextSize = mViewHeight / showCount;
